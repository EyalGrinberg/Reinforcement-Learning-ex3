...
im_R = np.zeros(shape=NUM_STATES)
V = np.random.uniform(0, 0.1, NUM_STATES)
# Empirical counts
trans_obs = np.zeros(shape=(NUM_STATES, NUM_ACTIONS, NUM_STATES))
reward_obs = np.zeros(shape=NUM_STATES)
###### END YOUR CODE ######
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    action = np.argmax([T[state][a].T @ V for a in range(NUM_ACTIONS)]) # Immediate reward isn't dependent on action
    ###### END YOUR CODE ######
    ...
    ###### BEGIN YOUR CODE ######
    trans_obs[state][action][new_state] += 1
    reward_obs[state] += R
    ###### END YOUR CODE ######
    ...
    if new_state == NUM_STATES - 1:
        ...
        for s in range(NUM_STATES):
            ...
            for a in range(NUM_ACTIONS):
                obs_sa = sum(trans_obs[s][a])
                if obs_sa > 0:
                    T[s][a] = trans_obs[s][a] / obs_sa
                obs_s += obs_sa
            if obs_s > 0:
                im_R[s] = reward_obs[s] / obs_s
        ###### END YOUR CODE ######
        ...
        ###### BEGIN YOUR CODE ######
        V_old = V
        V_new = np.array([im_R[s] + GAMMA*np.max([T[s][a].T @ V_old for a in range(NUM_ACTIONS)]) for s in range(NUM_STATES)])
        V_old = V_new
        V_new = np.array([im_R[s] + GAMMA*np.max([T[s][a].T @ V_old for a in range(NUM_ACTIONS)]) for s in range(NUM_STATES)])
        if np.max(np.abs(V_new-V_old)) < TOLERANCE:
            ...
            consecutive_no_learning_trials = 0
            while (np.max(np.abs(V_new-V_old)) >= TOLERANCE):
                V_old = V_new
                V_new = np.array([im_R[s] + GAMMA*np.max([T[s][a].T @ V_old for a in range(NUM_ACTIONS)]) for s in range(NUM_STATES)])
        V = V_new
        ###### END YOUR CODE ######
